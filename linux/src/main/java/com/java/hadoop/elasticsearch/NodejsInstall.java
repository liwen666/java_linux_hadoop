...
//			上传java文件 到hadoop用户目录
            ...
            for (Object str : ls) {
                ...
                if (cl.getLongname().endsWith("node-v10.15.0-linux-x64.tar.xz")) {
                    System.out.println(str + "   node 已经被安装");
                    mark = true;
                    break;
                }
            }
            if (!mark) {
                File javaFile = new File("G:\\开发安装包\\nodejs\\node-v10.15.0-linux-x64.tar.xz");
//                File javaFile = new File("H:\\hadoop-2.8.5.tar.gz");
            ...
            if (!mark) {
                ...
                String s2 = executeShell("xz -d node-v10.15.0-linux-x64.tar.xz");
                System.out.println(s2);
                String s3 = executeShell("tar xvf node-v10.15.0-linux-x64.tar -C /home/elasticsearch");
