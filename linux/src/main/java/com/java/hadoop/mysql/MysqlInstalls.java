...
public class MysqlInstalls {
    ...
    public void installJava() {
        try {
//			登录hadoop 的shell和ftp
            Login("mysql", "192.168.42.200", 22);
            execSession.setPassword("mysql");
            ftpsession.setPassword("mysql");
...
//			上传java文件 到hadoop用户目录
             ...
             */
            boolean mark = false;
            Vector ls = sftp.ls("/home/mysql");
            for (Object str : ls) {
                ChannelSftp.LsEntry cl = (ChannelSftp.LsEntry) str;
                if(cl.getLongname().endsWith("mysql-5.7.17-linux-glibc2.5-x86_64.tar")){
                    System.out.println(str+"   mysql 已经被安装");
            ...
            if(!mark){
                File javaFile = new File("D:\\idea2018workspace\\java_linux_hadoop\\linux\\src\\main\\java\\com\\java\\hadoop\\mysql\\mysql-5.7.17-linux-glibc2.5-x86_64.tar.gz");
           ...
           /****************************解压java文件***********************/
            if(!mark){
                String s1 = executeShell("mkdir -vp data");
                System.out.println(s1+"创建目录");
                String s2 = executeShell("tar zxvf mysql-5.7.17-linux-glibc2.5-x86_64.tar.gz  -C /home/mysql");
                System.out.println("解压 mysql_____________________"+s2);
            }
           /****************************解压java文件***********************/
//
//            /**修改profile文件  修改环境变量*/
